...
이 모듈은 "누가 요청했는지", "무슨 권한이 있는지", "세션이 유효한지"를 판단한다.  
...
- 인증 성공 시 사용자 식별자와 tenant/role 정보를 반환
- 권한이 없는 요청은 초기에 차단
...
- `src/jarvis_gateway/app.py`
  - FastAPI 앱과 인증/세션/사용자/감사 로그 엔드포인트
- `src/jarvis_gateway/auth.py`
  - 토큰 발급/폐기, principal 해석, role 검사
...
- 권한 모델을 단순 role 체크에서 세분화된 permission 체계로 고도화
