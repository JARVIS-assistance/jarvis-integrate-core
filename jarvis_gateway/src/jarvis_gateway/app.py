...
def create_app(db_path: str | None = None) -> FastAPI:
    ...
    def login(body: LoginRequest, request: Request):
        ...
        token = app.state.token_store.issue(user["id"], user["tenant_id"], user["role"])
        add_audit_log(
            app.state.db,
    ...
    def get_tenant_endpoint(tenant_id: str, principal: Principal = Depends(principal_from_header)):
        require_role(principal, {"admin", "member"})
    ...
    def create_user_endpoint(
        ...
        require_role(principal, {"admin"})
        if principal.tenant_id != body.tenant_id:
            raise HTTPException(status_code=403, detail="forbidden")
        tenant = get_tenant(app.state.db, body.tenant_id)
        if tenant is None:
            raise HTTPException(status_code=404, detail="tenant not found")
        user = create_user(app.state.db, body.tenant_id, body.username, body.password, body.role)
        add_audit_log(
            app.state.db,
            request.state.request_id,
            principal.user_id,
    ...
    def get_user_endpoint(user_id: str, principal: Principal = Depends(principal_from_header)):
        ...
        if user["tenant_id"] != principal.tenant_id:
            raise HTTPException(status_code=403, detail="forbidden")
        if principal.role != "admin" and user["id"] != principal.user_id:
            raise HTTPException(status_code=403, detail="forbidden")
        return user
    ...
    def get_session_endpoint(session_id: str, principal: Principal = Depends(principal_from_header)):
        ...
        if session["tenant_id"] != principal.tenant_id:
            raise HTTPException(status_code=403, detail="forbidden")
        if principal.role != "admin" and session["user_id"] != principal.user_id:
            raise HTTPException(status_code=403, detail="forbidden")
        return session
    ...
    def get_audit_logs(
        ...
        require_role(principal, {"admin"})
        return list_audit_logs(app.state.db, principal.tenant_id, min(limit, 100))
