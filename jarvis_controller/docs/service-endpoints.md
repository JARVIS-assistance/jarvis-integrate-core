...
현재 구현상 "회원가입" 전용 `/auth/signup`은 없다.

대신 실제 사용자 생성은 `jarvis_gateway`의 아래 엔드포인트가 담당한다.

- `POST /users`

다만 이 경로는 관리자 권한 기반의 user create 의미가 더 강하다. 그래서 외부 제품 API 기준으로는 아래 둘 중 하나로 정리하는 것이 맞다.

1. 당장은 `회원가입 = gateway /users`로 문서화
2. 이후 controller에 `POST /auth/signup`을 추가하고 내부적으로 gateway `/users`로 위임

현재 코드만 기준으로 보면 1번이 사실이고, 제품 관점 표준화까지 고려하면 2번이 더 자연스럽다.
